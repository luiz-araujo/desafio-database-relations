...
class ProductsRepository implements IProductsRepository {
  ...
  public async findAllById(products: IFindProducts[]): Promise<Product[]> {
    ...
    const orderList = await this.ormRepository.find({ id: In(productIds) });

    if (productIds.length !== orderList.length) {
  ...
  public async updateQuantity(
    ...
    const newProducts = productsData.map(productData => {
      ...
      if (productFind.quantity === 0) {
        throw new AppError('All products must have at least one');
      }
